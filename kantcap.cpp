...
class progressBar {
  ...
  public:
    ...
    progressBar(int bs, int percentStart,int barT ){
      blankSizeInit = bs;
    ...
    void clearBar(){
      bar = "";
      bar += " ";
      for(int i=0; i<blankSizeInit+5; i++){
        bar += " ";
      }
      bar += " ";
      std::cout << bar;
...
class videoCapture{
  ...
  public:
    ...
    int capturVideo(std::string vidFile, int frameCapStart, int frameCapStop){
      ...
      for (int i=0; i<formatContext->nb_streams*lastFrame+200; i++){
        //pauseToggle();
    ...
    void playVideo(std::string vidFile, int frameCap, int framePlaySt){
      ...
      for (int i=0; i<(formatContext->nb_streams+1)*lastFrame; i++){//
          ...
          if (packet->stream_index == videoStreamIndex) { //only video and ignor audio
            ...
            if (ret == 0) {//frame ok
              ...
              if (packet->pts >= framePlaySt){
                ...
                std::cout.flush();
                pgBar->updateBar((int)((packet->pts*100)/lastFrame));               
